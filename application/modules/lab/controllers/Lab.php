...
class Restaurant extends MY_Controller {
	...
	{		
		$data['setting'] = $this->Restaurant_Model->selectrow('setting',['id'=>'1']);
		if(!empty($_POST))
			{				
			...
			{ 
				$this->load->view('restaurant/loginhead');
				$this->load->view('restaurant/signup',$data);
			}
			else
			{
				...
				#pr($insert);
				$insert_into = $this->Restaurant_Model->insert('users',$insert);
				...
				{ 
					...
					$restaurant_address_id = $this->Restaurant_Model->insert('restaurant_address',$restaurant_address);
					if(!empty($restaurant_address_id))
					{
						...
						$this->Restaurant_Model->insert('restaurant_timings',$restaurant_timings);
	...
	public function otp()
	{
		if($this->session->has_userdata('user_id'))
		{
			...
			{ 
				$this->load->view('restaurant/loginhead');
				$this->load->view('restaurant/otp');
			}
			else
			{
				$get_otp = $this->input->post('otp');
				$insert_into = $this->Restaurant_Model->selectrow('users',['otp'=>$get_otp,'id'=>$user_id,'otp_status'=>'1','is_verified'=>'0']);
				if($insert_into)
				{
					$this->Restaurant_Model->update('users',['otp_status'=>'0','is_verified'=>'1'],['id'=>$user_id]);
	...
	public function create_password()
	{
		if($this->session->has_userdata('new_pass'))
		{
			...
			{ 
				$this->load->view('restaurant/loginhead');
				$this->load->view('restaurant/new_password');
			}
			else
			{
				$updated_pass = password_hash($this->input->post('password'),PASSWORD_DEFAULT);
				$update_into = $this->Restaurant_Model->update('users',['password'=>$updated_pass],['id'=>$user_id]);
				if($update_into)
				{
	...
	public function login()
	{
		if(!empty($_POST))
			...
			else
			{
				...
				$user_row = $this->Restaurant_Model->selectrow('users',['mobile'=>$mobile,'usertype'=>'2','is_verified'=>'1','status'=>'1','is_deleted'=>'0']);

				$hash = @$user_row->password;
			
				if($user_row->user_status == '0')
				{
					$this->session->set_flashdata('error',"Your Account is pending for Approval please wait.....");
					redirect($_SERVER['HTTP_REFERER']);
				}
				elseif ($user_row->user_status == '1' && password_verify($password, $hash)) {
					$this->session->set_userdata('loginrestaurant',$user_row);
					redirect('restaurant/dashboard');
				} else {					
	...
	public function forget()
	{
		die('on_process.........');
			...
			{ 
				$this->load->view('restaurant/loginhead');
				$this->load->view('restaurant/forget');
			}
			else
			{
				$mobile = $this->input->post('mobile');
				$user_row = $this->Restaurant_Model->selectrow('users',['mobile'=>$mobile,'usertype'=>'1','user_status !='=>'3','status'=>'1','is_deleted'=>'0']);
				if(!empty($user_row))
				{
					...
					$update_into = $this->Restaurant_Model->update('users',$update,['id'=>$user_row->id]);
	...
	public function product()
	{
		...
		$data['product']=$this->Restaurant_Model->select('products',['vender_id'=>$restaurant_id,'is_approved'=>'1','is_deleted'=>'0']);
		$this->load->view('restaurant/product',$data);
	}	
	...
	public function addProduct()
	{
		...
		$data['category']=$this->Restaurant_Model->select('category',['type'=>'1','status'=>'1','is_deleted'=>'0']);
		if(!empty($_POST))
		{
			...
			if($this->form_validation->run()==FALSE){
				...
				$this->load->view('restaurant/addproduct',$data);
			}else{
				...
				$insert = $this->Restaurant_Model->insert('products',$data);
				if($insert)
				{
					foreach($image_data as $v)
					{
						$this->Restaurant_Model->insert('item_images',['user_id'=>$insert,'image'=>$v]);
					}
	...
	public function get_subcat()
	{
		...
		$data=$this->Restaurant_Model->select('sub_category',['category_type'=>'1','is_deleted'=>'0','category_id'=>$cat_name]);
		if($data)
		{
			$status = true;
		}
	...
	public function item_status($ide)
    {
        ...
        if(!empty($ide))
        {
			...
			$check = $this->Restaurant_Model->selectrow('products',['id'=>$id]);
			$value = ($check->status==1)?'0':'1';
			$msg=($value=='1')?'Product active successfully':'Product inactive successfully';
			
			$this->Restaurant_Model->update('products',['status'=>$value],['id'=>$check->id]);
	...
	public function delete_item($ide)
	{
		checkrestaurantlogin();
		$id=base64_decode($ide);
		$this->Restaurant_Model->update('products',['is_deleted'=>'1'],['id'=>$id]);
	...
	public function item_image($id)
	{
		checkrestaurantlogin();
		$id = base64_decode($id);
		$data['products_image']=$this->Restaurant_Model->select('item_images',['user_id'=>$id,'is_deleted'=>'0']);
		$this->load->view('restaurant/product_images',$data);
		// if(!empty($_POST))
		// {
		// 	$this->form_validation->set_rules('name', 'Name', 'trim|required');
		...
		// 					);
		// 		$insert = $this->Restaurant_Model->update('products',$data,['id'=>$id]);
		// 		if($insert)
		// 		{				
		...
		$id = base64_decode($ide);	
			if(!empty($_FILES['more_img']['name'][0]))
			{
				...
				if($image_data)
				{
					foreach($image_data as $v)
					{
						$this->Restaurant_Model->insert('item_images',['user_id'=>$id,'image'=>$v]);
					}
	...
	public function delete_item_image($ide)
	{
		...
		$this->Restaurant_Model->update('item_images',$data,['id'=>$id]);
	...
	public function edit_item($id)
	{
		...
		$id = base64_decode($id);
		$restaurant_id = $this->session->userdata('loginrestaurant')->id;
		$data['products']=$this->Restaurant_Model->selectrow('products',['id'=>$id]);
		$data['category']=$this->Restaurant_Model->select('category',['type'=>'1','status'=>'1','is_deleted'=>'0']);
		$data['sub_category']=$this->Restaurant_Model->select('sub_category',['category_type'=>'1','status'=>'1','is_deleted'=>'0','category_id'=>$data['products']->cat_id]);
		...
		if(!empty($_POST))
		{
			...
			if($this->form_validation->run()==FALSE){
				$this->load->view('restaurant/editproduct',$data);
			}else{				
	...
	public function orders_request()
	{
		checkrestaurantlogin();
		$restaurant_id = $this->session->userdata('loginrestaurant')->id;
			$data['orders_request'] = $this->Restaurant_Model->select('orders',['is_approved'=>'0','vendor_type '=>'1','vendor_id'=>$restaurant_id]);		
			$this->load->view('restaurant/order_request',$data);		
	}
	...
	public function ongoing_orders()
	{
		checkrestaurantlogin();
		$restaurant_id = $this->session->userdata('loginrestaurant')->id;
			$data['ongoing_orders'] = $this->Restaurant_Model->select_ongoing_orders('orders',$restaurant_id);		
			$this->load->view('restaurant/ongoing_orders',$data);		
	}
	...
	public function complete_orders()
	{
		checkrestaurantlogin();
		$restaurant_id = $this->session->userdata('loginrestaurant')->id;
			$data['complete_orders'] = $this->Restaurant_Model->select('orders',['order_status'=>'3','vendor_type '=>'1','vendor_id'=>$restaurant_id]);		
			$this->load->view('restaurant/complete_orders',$data);		
	}
	...
	public function view_request_orders_details($id)
	{
		...
		$data['orders']=$this->Restaurant_Model->selectrow('orders',['id'=>$id]);
		$data['vender_details']=$this->Restaurant_Model->selectrow('users',['id'=>$data['orders']->vendor_id]);
		$product_details = json_decode($data['orders']->product_details); 
		foreach($product_details as $key) 
		{ 	
			$datadetails = $this->Restaurant_Model->selectrow('products',['id'=>$key->product_id]);
		...
		$this->load->view('restaurant/view_request_orders_details',$data);
	}
	...
	public function action_orders_request($action='',$id='')
	{
		...
		{			
			...
			if($action === '1')
			{
				$this->Restaurant_Model->update('orders',['order_status'=>'1'],['id'=>$id]);	
			}			
			$this->Restaurant_Model->update('orders',['is_approved'=>$action],['id'=>$id]);
		}
		else
		{
	...
	public function update_delivery_status()
	{
		...
		{		
			...
			$update = $this->Restaurant_Model->update('orders',['order_status'=>$d_status],['id'=>$row_id]);
			if($update)
			{				
				$status = true;
			}
	...
	public function update_profile()
	{
		...
		$data['profile_details'] = $this->Restaurant_Model->selectrow('users',['id'=>$id,'usertype'=>'2','is_verified'=>'1','user_status'=>'1','status'=>'1','is_deleted'=>'0']);
		$data['address'] = $this->Restaurant_Model->selectrow('restaurant_address',['restaurant_id'=>$data['profile_details']->id,'status'=>'1','is_deleted'=>'0']);
		$data['timming'] = $this->Restaurant_Model->selectrow('restaurant_timings',['restaurant_id'=>$data['profile_details']->id,'status'=>'1','is_deleted'=>'0']);
		...
		{	
			...
			if($this->form_validation->run()==FALSE){
				$this->load->view('restaurant/edit_profile',$data);
			}
			else
			{
				if (!empty($_FILES['id_proof']['name'])) 
				...
				$this->Restaurant_Model->update('users',$insert,['id'=>$id]);
				$this->Restaurant_Model->update('restaurant_address',$restaurant_address,['restaurant_id'=>$id]);
				$this->Restaurant_Model->update('restaurant_timings',$restaurant_timings,['restaurant_id'=>$id]);
	...
	public function change_password()
	{
		...
		{	
			...
			{
				$restaurant_pass = $this->Restaurant_Model->selectrow('users',['id'=>$id]);
				...
				if(password_verify($this->input->post('old_password'),$restaurant_pass->password))
				{
					...
					$this->Restaurant_Model->update('users',$data2,['id'=>$id]);
					$this->session->set_flashdata('message',"Password Updated");
					return redirect($_SERVER['HTTP_REFERER']);
				}
				else
	...
	public function faq()
	{
		$user_id = $this->session->userdata('loginrestaurant')->id;			
			...
			$data['insert_into'] = $this->Restaurant_Model->selectrow('faq',['vendor_id'=>$user_id]);

			if($this->form_validation->run() === FALSE)
			{ 
				$this->load->view('restaurant/faq',$data);
			}
			else
			{
				...
				if($data['insert_into'])
				{
					$this->Restaurant_Model->update('faq',['faq '=>$faq],['vendor_id'=>$user_id]);				
					$this->session->set_flashdata('message',"FAQ's Updated Successfully");
					return redirect($_SERVER['HTTP_REFERER']);		
				}
				else
				{
					$this->Restaurant_Model->insert('faq',['faq '=>$faq,'vendor_id'=>$user_id]);
	...
	public function about_us()
	{
		$user_id = $this->session->userdata('loginrestaurant')->id;			
			...
			$data['insert_into'] = $this->Restaurant_Model->selectrow('about_us',['vendor_id'=>$user_id]);

			if($this->form_validation->run() === FALSE)
			{ 
				$this->load->view('restaurant/about_us',$data);
			}
			else
			{
				...
				if($data['insert_into'])
				{
					$this->Restaurant_Model->update('about_us',['about_us '=>$about_us],['vendor_id'=>$user_id]);				
					$this->session->set_flashdata('message',"About Us Updated Successfully");
					return redirect($_SERVER['HTTP_REFERER']);		
				}
				else
				{
					$this->Restaurant_Model->insert('about_us',['about_us '=>$about_us,'vendor_id'=>$user_id]);
